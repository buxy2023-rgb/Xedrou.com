...
router.post("/country", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (!/^[A-Z]{2}$/.test(country_code)) {
    return res.status(400).json({ error: "A valid country selection is required" });
  }
  if (!/^[A-Z]{3}$/.test(currency_code)) {
    return res.status(400).json({ error: "A valid currency is required" });
  }
  if (!isValidTimeZone(time_zone)) {
    return res.status(400).json({ error: "A valid time zone is required" });
  }
  ...
  await supabaseAdmin
    .from("xedruo_users")
    .update({ country_code, currency_code, time_zone, locale, updated_at: new Date().toISOString() })
    .eq("id", req.user!.id);

  try {
    await supabaseAdmin.auth.admin.updateUserById(req.user!.id, {
      user_metadata: { country_code, currency_code, time_zone, locale },
    });
  } catch {
    // Profile data remains the source of truth if metadata enrichment fails.
  }

  res.status(201).json({
    profile,
    registration: { country_code, currency_code, time_zone, locale },
  });
});
