...
export default function PhoneRegistration() {
  ...
  useEffect(() => {
    ...
    base44.auth.me().then((user) => {
      ...
      setLoading(false);
    }).catch(() => { if (mounted) { setError("Please complete Google sign-in before adding your phone number."); setLoading(false); } });
  ...
  async function submit() {
    ...
    try {
      await base44.auth.savePhoneNumber(value);
      navigate(`/registration/country?next=${encodeURIComponent(next)}`, { replace: true });
  ...
  return <div className="min-h-screen bg-slate-950 px-5 py-10 text-white md:py-16"><div className="mx-auto flex min-h-[75vh] max-w-md items-center"><div className="w-full rounded-3xl border border-white/10 bg-white/5 p-7 md:p-9">
    ...
    <div className="mt-7 text-xs uppercase tracking-[.25em] text-cyan-300">Step 1 of 2</div><h1 className="mt-2 text-3xl font-black">Add your phone number</h1><p className="mt-3 text-sm leading-6 text-slate-400">Your phone becomes the account contact used across the Xedruo ecosystem. We keep your Google account as your primary sign-in.</p>
    ...
    <button onClick={submit} disabled={saving} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-300 px-5 py-4 font-bold text-slate-950 disabled:opacity-50">{saving ? <Loader2 className="animate-spin" size={18}/> : null}{saving ? "Saving…" : "Continue to country"}<ArrowRight size={18}/></button>
  </div></div></div>;
}
