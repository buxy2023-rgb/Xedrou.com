...
export default function WorkerAccess(){
  ...
  return <div className="min-h-screen bg-slate-950 text-white p-5 md:p-10"><div className="mx-auto max-w-5xl"><div className="mb-8"><div className="text-xs uppercase tracking-[.3em] text-cyan-300">Xedruo workforce access</div><h1 className="mt-2 text-4xl font-bold">Select your function</h1><p className="mt-3 text-slate-400">Choose a function and its workstation dashboard opens immediately. You can change the company from the top-right of the workstation.</p></div><div className="rounded-3xl border border-white/10 bg-white/5 p-6"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{roles.map(({id,name,desc,Icon,dashboard,route})=><button key={id} onClick={()=>openFunction({id,name,route})} className="rounded-2xl border border-white/10 bg-black/20 p-5 text-left transition hover:border-cyan-300/40 hover:bg-cyan-300/10"><Icon className="mb-3 text-cyan-300" size={22}/><div className="font-semibold text-lg">{name}</div><div className="mt-1 text-xs text-slate-500">{desc}</div><div className="mt-4 flex items-center gap-2 text-xs text-cyan-300"><BriefcaseBusiness size={14}/> {dashboard}</div></button>)}</div></div><p className="mt-4 text-center text-xs text-slate-600">Company-sensitive data remains subject to server-side authorization.</p></div></div>;
}
