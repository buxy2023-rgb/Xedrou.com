...
export default function WorkerAccess() {
  ...
  if (selectedRole) {
    return <div className="min-h-screen bg-slate-950 p-5 text-white md:p-10"><div className="mx-auto max-w-lg">
      <button onClick={() => { setSelectedRole(null); setError(""); }} className="mb-6 flex items-center gap-2 text-sm text-slate-400"><ArrowRight className="rotate-180" size={16}/> Back to Staff & Manager portal</button>
      <div className="rounded-3xl border border-cyan-300/20 bg-white/5 p-7">
        ...
        {selectedRole.id !== "governor" && <><label className="mt-7 block text-sm text-slate-300">Username</label><input value={username} onChange={e => setUsername(e.target.value)} autoCapitalize="none" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900 p-4 outline-none" placeholder="Enter username"/></>}
        <label className="mt-4 block text-sm text-slate-300">Password</label><input value={password} onChange={e => setPassword(e.target.value)} type="password" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900 p-4 outline-none" placeholder="Enter password" onKeyDown={e => e.key === "Enter" && login()}/>
        ...
        <p className="mt-4 text-xs leading-5 text-slate-500">Access is permission-controlled server-side. Developers do not select Elinit or a project here; after authentication, the developer opens their assigned workstation.</p>
      </div>
    </div></div>;
  }
