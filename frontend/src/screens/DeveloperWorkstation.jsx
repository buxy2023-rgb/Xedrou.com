...
import { AppWindow, ChevronDown, Code2, ExternalLink, FileUp, FolderOpen, Globe2, Image as ImageIcon, Link2, MessageSquare, MoreHorizontal, PackagePlus, Plus, Send, Upload, Trash2 } from "lucide-react";
...
function headers(extra = {}) {
  return { apikey: STORAGE_KEY, Authorization: `Bearer ${STORAGE_KEY}`, ...extra };
}

async function listProjectFiles(projectId) {
  const res = await fetch(`${STORAGE_URL}/storage/v1/object/list/${BUCKET}`, {
    method: "POST",
    headers: headers({ "Content-Type": "application/json" }),
    body: JSON.stringify({ prefix: `projects/${projectId}/`, limit: 1000, offset: 0, sortBy: { column: "created_at", order: "desc" } }),
  });
  if (!res.ok) throw new Error(await res.text());
  const rows = await res.json();
  return (rows || []).filter(x => x.name).map(x => ({
    id: x.id || x.name,
    name: x.name,
    path: `projects/${projectId}/${x.name}`,
    type: x.metadata?.mimetype || "application/octet-stream",
    size: x.metadata?.size || 0,
    url: `${STORAGE_URL}/storage/v1/object/public/${BUCKET}/projects/${encodeURIComponent(projectId)}/${encodeURIComponent(x.name)}`,
  }));
}
...
async function uploadProjectFiles(projectId, incoming) {
  ...
  for (const file of incoming) {
    ...
    const res = await fetch(`${STORAGE_URL}/storage/v1/object/${BUCKET}/${path}`, {
      method: "POST",
...
export default function DeveloperWorkstation(){
  ...
  useEffect(()=>{
    ...
    listProjectFiles(selected.id).then(rows=>{ if(!cancelled) setFiles(rows); }).catch(err=>{ if(!cancelled) setFileError(`Storage connection error: ${err.message}`); }).finally(()=>{ if(!cancelled) setLoadingFiles(false); });
  ...
  async function handleFiles(e){
    ...
    try { const uploaded=await uploadProjectFiles(selected.id,incoming); setFiles(f=>[...uploaded,...f]); setChat(c=>[...c,{role:"assistant",text:`Uploaded ${uploaded.length} file${uploaded.length===1?"":"s"} to ${selected.name} storage.`}]); }
    catch(err){ setFileError(`Upload failed: ${err.message}`); }
    finally { setUploading(false); e.target.value=""; }
  }
  async function removeFile(file){
    try { await deleteProjectFile(file.path); setFiles(f=>f.filter(x=>x.path!==file.path)); }
    catch(err){ setFileError(`Delete failed: ${err.message}`); }
  }
