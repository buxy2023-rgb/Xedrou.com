...
export default function CompanyGateway() {
  ...
  useEffect(() => {
    if (!isLoadingAuth && isAuthenticated && user && company && !verificationPending) {
      openCompany(user);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [company, isAuthenticated, isLoadingAuth, user, verificationPending]);
  ...
  return <div className="min-h-screen bg-slate-950 px-5 py-10 text-white md:py-16"><div className="mx-auto flex min-h-[75vh] max-w-md items-center"><div className="w-full rounded-3xl border border-white/10 bg-white/5 p-7 shadow-2xl md:p-9">
    ...
    <div className="mt-8 text-xs uppercase tracking-[.25em] text-cyan-300">Xedruo company</div><h1 className="mt-2 text-3xl font-black">{name}</h1><p className="mt-3 text-sm leading-6 text-slate-400">{description}. Access your customer dashboard securely.</p>
    ...
    <div className="mt-5 flex items-center gap-3 text-xs text-slate-600"><div className="h-px flex-1 bg-white/10"/><span>unified Xedruo account</span><div className="h-px flex-1 bg-white/10"/></div>
